...
def compute_binary_CE_loss(label_predictions, mortality_label,
                           pos_weight=0.0, label_smoothing=0.0,
                           focal_gamma=0.0):
    ...
    # Single-class mini-batches are possible on imbalanced ICU datasets;
    # BCE is still well-defined, so keep the training output quiet.
    ...
    if label_smoothing > 0.0:
        ...
        mortality_label = mortality_label * (1.0 - smooth) + 0.5 * smooth

    pos_weight_tensor = None
    if pos_weight is not None and float(pos_weight) > 0.0:
        pos_weight_tensor = torch.tensor([float(pos_weight)]).to(label_predictions)
    ...
    ce_loss = F.binary_cross_entropy_with_logits(
        ...
        reduction='none')
    if focal_gamma > 0.0:
        prob = torch.sigmoid(label_predictions)
        pt = prob * mortality_label + (1.0 - prob) * (1.0 - mortality_label)
        ce_loss = ce_loss * torch.pow(1.0 - pt, float(focal_gamma))
...
def get_mse(self, truth, pred_y, mask = None):
    ...
    #print(truth.shape,pred_y.shape,mask.shape)
