...
import utils
from training.metrics import compute_log_normal_pdf
...
class SegmentGraphPosterior(nn.Module):
    def __init__(
        ...
        self.pool = pool
        self.eps = eps
        self.softmax_temperature = max(float(softmax_temperature), eps)
        self.topk = max(0, min(int(topk), self.nodes))
    ...
    def _sparsify(self, adjacency):
        if self.topk <= 0 or self.topk >= self.nodes:
            return adjacency
        values, indices = adjacency.topk(self.topk, dim=-1)
    ...
    def _sample_global_graph(self, segment_mu, segment_std, weights):
        ...
        global_adj = torch.softmax(global_mu / self.softmax_temperature, dim=-1)
        global_adj = self._sparsify(global_adj)
        global_kl = 0.5 * (
    ...
    def forward(self, x, mask, times=None):
        ...
        for start, end in bounds:
            ...
            attn = torch.softmax(logits / self.softmax_temperature, dim=-1)
            attn = self._sparsify(attn)

            h = self.graph_encoder(attn) + self.node_encoder(node_features)
            mu = self.mu_net(h)
            std = F.softplus(self.std_net(h)) + self.eps
            adj = torch.softmax(mu / self.softmax_temperature, dim=-1)
...
class GSNF(nn.Module):
    def __init__(
        ...
        self.graph_posterior = SegmentGraphPosterior(
            ...
            softmax_temperature=getattr(args, "graph_softmax_temperature", 0.5),
            topk=getattr(args, "graph_topk", 5))
